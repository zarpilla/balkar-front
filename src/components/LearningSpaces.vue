  ...
  <div class="learning-spaces" v-if="loaded">
    ...
    <div v-for="space in mySpaces" :key="space.id">
      <RouterLink :to="`/space/${space.uid}`" class="btn btn-tertiary mb-4">
        {{ space.name }}
        <span v-if="space.completedPct !== null">{{ '(' + (space.completedPct * 100).toFixed(0) + '%)' }}</span>
        <svg
          width="19"
          height="15"
          viewBox="0 0 19 15"
    ...
    <h3 v-if="otherSpaces.length > 0" class="mt-5 mb-4">{{ $t("tots-els-espais-d-aprenentatge") }}</h3>
