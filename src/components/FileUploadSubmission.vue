...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const addFile = (e: Event) => {
      ...
      // formData.append('moduleId', props.moduleId as string)
      // formData.append('learning_space', props.learningSpace as string)
      // formData.append('users_persmissions_user', '1' as string)
      // formData.append('field', 'file' as string)
    ...
    const filesChange = (event: any, fieldName: string, xfileList: FileList) => {
      ...
      Array.from(Array(xfileList.length).keys()).map((x) => {
        formData.append('file', xfileList[x], xfileList[x].name);
      });

      const data = { moduleId: props.moduleId, learning_space: props.learningSpace, users_permissions_user: '0', field: 'file'}

      // data.append('moduleId', props.moduleId as string)
      // formData.append('learning_space', props.learningSpace as string)
      // formData.append('users_persmissions_user', '1' as string)
      // formData.append('field', 'file' as string)
